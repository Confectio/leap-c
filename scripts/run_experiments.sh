...
for j in "${!seeds[@]}"; do
    ...
    screen -dm -S "${seed}_experiment_${actual_ind}" bash -l -c "source ~/.bashrc && micromamba activate leapc_env && python run_sac_fop.py --env '${environment}' --seed ${seed} --device '${device}' -wg '${environment}_${g_add}' -wt '${tag}' -w True"
